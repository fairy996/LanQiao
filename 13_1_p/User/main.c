...
unsigned int temp;
unsigned char Time_Now[3] = {0x00, 0x00, 0x00};
// 时间数据
unsigned char Time[3] = {0x07, 0x59, 0x30};
unsigned int time_5s;
unsigned char time_100ms;

// 标志位
unsigned char state = 0; // 界面标志 0为温度 1为时间 2为参数
unsigned char mode = 0;  // 控制模式 0为温度 1为时间
unsigned char L3_Light_Flag = 0;
unsigned char Time_View_Flag = 0;//0表示显示时和分,1表示显示分和秒
unsigned char Int_Time_Flag = 0; //表示当前为整点
unsigned char Trigger_Flag = 0;  //0表示此时继电器未达到触发状态，1表示继电器吸合
...
void NiXie_pro()
{
    ...
    if(mode == 0 && (temp>temp_para))
        Trigger_Flag = 1;
    if(mode == 1 && Time_Now[1] == 0 && Time_Now[2] == 0)
        Trigger_Flag = 1;
    NiXie_Value[0] = 17;
